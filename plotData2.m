...
dataFileRaw = importdata('dataLog.txt');
...
% mean(dt);
figure();
subplot(4,1,1);plot(volt/1024*5,'k','linewidth',2);xlabel('time [s]');ylabel('voltage [V]');grid on; hold on;
subplot(4,1,2);plot(stimul,'k','linewidth',2);xlabel('time [s]');ylabel('stimulus on/off');grid on; hold on;
...
figure();
plot(stimul*4.5,'r');hold on; grid on;
plot(volt/1024*5);legend('stimulation','FSR output');xlabel('time [s]');